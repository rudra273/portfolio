...
import BlackHolePhenomenon from './BlackHolePhenomenon'
...
type FlightRef = MutableRefObject<FlightState>
...
function WarpLines({ flightRef, count = 42 }: { flightRef: FlightRef; count?: number }) {
...
function FeatureStars({ flightRef }: { flightRef: FlightRef }) {
...
function SpiralGalaxy({ flightRef }: { flightRef: FlightRef }) {
  const groupRef = useRef<THREE.Group>(null)
  const glowTexture = useMemo(() => createGlowTexture(), [])
  const starTexture = useMemo(() => createStarPointTexture(), [])
...
function RingedPlanet({ flightRef }: { flightRef: FlightRef }) {
  const groupRef = useRef<THREE.Group>(null)
...
function IcePlanet({ flightRef }: { flightRef: FlightRef }) {
  const groupRef = useRef<THREE.Group>(null)
  const planetRef = useRef<THREE.Mesh>(null)
  const { viewport } = useThree()
